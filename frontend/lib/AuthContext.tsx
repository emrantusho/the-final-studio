...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const verifyToken = useCallback(async () => {
    ...
    if (!token) {
        setIsLoading(false);
        // If we're not on the login page and have no token, redirect.
  ...
  // Run verification on initial load and on every page navigation
  useEffect(() => {
    verifyToken();
  }, [pathname]);
  ...
  const login = async (username: string, password: string) => {
    ...
    setUser(userData);
    router.push('/'); // Go to the main chat page after login
  };
  ...
  // While verifying the token, show a global loading screen
  if (isLoading) {

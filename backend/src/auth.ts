...
authApp.post('/login', zValidator('json', LoginSchema), async (c) => {
    ...
    const sessionId = crypto.randomUUID(); const expires = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    if (!c.env.SESSION_SECRET) { return c.json({ error: 'Session secret not configured' }, 500); }
